...
- **内存消耗**: 根据启用的功能组合，MemDiag 对目标 JVM 的额外内存占用如下：

| 模式 / 功能组合 | 适用场景 | 建议预留内存 | 核心资源消耗项 |
|:---|:---|:---|:---|
| **JMX 模式** (默认) | 本地临时诊断、堆/线程分析 | < 32MB | JMX 响应解析缓冲区 |
| **Agent 模式** (仅 HTTP 服务) | 跨机器 HTTP 远程诊断 | 32MB - 64MB | HTTP 服务与 Agent 上下文 |
| **Agent 模式** (+字节码插桩) | 实时分配追踪、方法执行监控 | 64MB - 128MB | 环形缓冲区 (RingBuffer)、方法统计表 |
| **Agent 模式** (+JVMTI 集成) | 深层原生内存追踪 (Linux) | 128MB - 256MB | 原生符号缓存、JNI 事件队列 |

> **提示**: 默认配置下，MemDiag 采用**固定大小的环形缓冲区**和**原子计数采样**，确保内存开销可控。可以通过 `samplingRate` 和 `ringBufferSize` 参数进一步压缩在高负载环境下的内存占用。
