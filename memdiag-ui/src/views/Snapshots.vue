...
const handleCreate = async () => {
  ...
  if (result.success) {
    toast.add({ severity: 'success', summary: 'Success', detail: 'Snapshot created', life: 3000 })
...
const handleDelete = async (id) => {
  ...
  if (result.success) {
    toast.add({ severity: 'info', summary: 'Deleted', detail: 'Snapshot removed', life: 3000 })
  }
}
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Snapshots</h1>
        <p class="text-slate-500 mt-1 text-sm">Capture and manage JVM heap and thread states.</p>
      </div>
    </div>
            ...
            >
              <template #empty>
                <div class="text-center py-12">
                  ...
                  <p class="text-slate-400">No snapshots found for this connection.</p>
                </div>
              </template>
              ...
              <Column field="size" header="Size" sortable class="text-right">
                <template #body="slotProps">
          ...
          <template #content>
            <div class="space-y-4">
              <p class="text-xs text-slate-500 leading-relaxed">
                A snapshot captures the complete heap histogram and thread dump of the target JVM.
              </p>
              <div class="flex flex-col gap-2">
                <label class="text-sm font-semibold text-slate-700">Optional Name</label>
                <InputText v-model="newSnapshotName" placeholder="e.g. before-gc-optimization" class="rounded-xl bg-slate-50 border-slate-100" />
              </div>
              <Button 
                label="Capture Now" 
          ...
          <div class="relative z-10">
            ...
            <div class="space-y-4">
              <div class="flex justify-between items-end">
                <span class="text-sm text-slate-400">Used Space</span>
              ...
              <p class="text-[10px] text-slate-500">Snapshots are stored on the MemDiag server node.</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-else class="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-dashed border-slate-200">
      ...
      <h3 class="text-xl font-bold text-slate-900">Manage Snapshots</h3>
